...
def learing_rate_scheduler():
    ...
    lr = np.power(d_model, -1.1) * min(np.power(step_num, -0.5),
...
if args.resume:
    ...
    model_gen = model_denoise_autoenc_attack.AutoencModel(ntokens, args.emsize,
                                                          args.encoding_layers,
                                                          args.pos_drop,
                                                          args.dropout_transformer,
                                                          args.dropouti, args.dropoute,
                                                          args.tied, args.attn_heads)
...
def train():
    ...
    while i < train_real_data.size(0) - 1 - 1:
        if not args.fixed_length:
            ...
            data_real, targets_real = get_batch_no_msg(train_real_data,
                                                       i,
                                                       args,
                                                       seq_len=seq_len)
            data_fake, targets_fake = get_batch_no_msg(train_fake_data,
                                                       fake_batch_itr,
                                                       args,
                                                       seq_len=seq_len)
        else:
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            print('| epoch {:3d} | {:5d}/{:5d} batches | gen lr {:05.8f} | '
                  'reconst loss {:5.4f}'.format(epoch, batch,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if 't0' in optimizer_gen.param_groups[0]:
            ...
            print('| end of epoch {:3d} | time: {:5.2f}s | '
                  'val reconst loss {:5.2f}'.format(epoch,
