...
def compare_msg_whole(msgs, msg_out):
    correct = np.count_nonzero(
        np.sum(np.equal(msgs.detach().cpu().numpy().astype(int),
                        msg_out.detach().cpu().numpy().astype(int)),
               axis=1) == args.msg_len * args.msgs_segment)
    return correct


def compare_msg_bits(msgs, msg_out):
    correct = np.count_nonzero(
        np.equal(msgs.detach().cpu().numpy().astype(int),
                 msg_out.detach().cpu().numpy().astype(int)) == True)
    return correct
    ...
    for i in range(0, data_source.size(0) - args.bptt, args.bptt):
        ...
        with torch.no_grad():
            ...
            real_correct = real_correct + np.count_nonzero(
                np.equal(label.detach().cpu().numpy().astype(int),
                         real_out_label.detach().cpu().numpy().astype(int)) == True)
            ...
            for beam in range(0, args.samples_num):
                hidden = langModel.init_hidden(batch_size)
                word_idx = get_idx_from_logits(candidates_soft_prob[beam], data.size(0),
                                               batch_size)
                output_text = ''
                orig_text = ''
                for k in range(0, data.size(0)):
                    output_text = output_text + corpus.dictionary.idx2word[word_idx[
                        k, 0]] + ' '
            ...
            # meteor_score_selected = meteor_score([orig_text],
            #                                      output_text_beams[best_beam_idx])
            ...
            fake_correct = fake_correct + np.count_nonzero(
                np.equal(label.detach().cpu().numpy().astype(int),
                         fake_out_label.detach().cpu().numpy().astype(int)) == True)
            y_label.append(label.detach().cpu().numpy().astype(int)[0, 0])
        ...
        f_metrics.write(
            str(meteor_score_selected) + ',' + str(bert_diff_selected) + ',' +
            str(lm_loss_selected) + '\n')
    Fscore = f1_score(y_label, y_out)
    p_value_smaller = sum(i < 0.01 for i in p_value)
    return total_loss_lm / batch_count, correct_msg_count / tot_count, correct_msg_count_bits / tot_count_bits, meteor_tot / batch_count, l2_distances / batch_count, fake_correct / batch_count, real_correct / batch_count, Fscore, np.mean(
        p_value), p_value_smaller / len(p_value)
...
f = open('val_out_lm.txt', 'w')
f_metrics = open('val_out_metrics_lm.txt', 'w')
val_lm_loss, val_correct_msg, val_correct_bits_msg, val_meteor, val_l2_sbert, val_correct_fake, val_correct_real, val_Fscore, val_pvalue, val_pvalue_inst = evaluate(
    val_data, f, eval_batch_size)

print('-' * 150)
print(
    '| validation | lm loss {:5.2f} | msg accuracy {:5.2f} | msg bit accuracy {:5.2f} |  meteor {:5.4f} | SentBert dist. {:5.4f} | fake accuracy {:5.2f} | real accuracy {:5.2f} | F1 score {:.5f} | P-value {:.9f} | P-value inst {:5.2f}'
    .format(val_lm_loss, val_correct_msg * 100, val_correct_bits_msg * 100, val_meteor,
            val_l2_sbert, val_correct_fake * 100, val_correct_real * 100, val_Fscore,
            val_pvalue, val_pvalue_inst * 100))
print('-' * 150)
f.close()
...
test_lm_loss, test_correct_msg, test_correct_bits_msg, test_meteor, test_l2_sbert, test_correct_fake, test_correct_real, test_Fscore, test_pvalue, test_pvalue_inst = evaluate(
    test_data, f, test_batch_size)

print('=' * 150)
print(
    '| test | lm loss {:5.2f} | msg accuracy {:5.2f} | msg bit accuracy {:5.2f} | meteor {:5.4f} | SentBert dist. {:5.4f} | fake accuracy {:5.2f} | real accuracy {:5.2f} | F1 score {:.5f} | P-value {:.9f} | P-value inst {:5.2f}'
    .format(test_lm_loss, test_correct_msg * 100, test_correct_bits_msg * 100,
            test_meteor, test_l2_sbert, test_correct_fake * 100, test_correct_real * 100,
            test_Fscore, test_pvalue, val_pvalue_inst * 100))
print('=' * 150)
f.close()
