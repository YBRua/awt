...
parser.add_argument('--modify_prob', type=float, default=0.05,
                    help='Prob to remove words')
...
def evaluate(data_source, out_file, batch_size=10, on_train=False):
    ...
    for i in range(0, data_source.size(0) - args.bptt, args.bptt):
        ...
        with torch.no_grad():
            ...
            new_sent_list, new_sent_str, new_bert_diff, new_meteor, new_words_idx = modify_words_main(best_beam_text, orig_text)		

            best_beam_emb =  model_gen.forward_sent(new_words_idx,msgs,args.gumbel_temp,only_embedding=True)		
