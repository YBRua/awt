...
def train():
    ...
    while i < train_data.size(0) - 1 - 1:
        if not args.fixed_length:
            ...
            data, data_noise, targets = get_batch_noise(train_data, i, args, ntokens, seq_len=seq_len)
        else:
            seq_len = args.bptt
            data, data_noise, targets = get_batch_noise(train_data, i, args,ntokens, seq_len=None)
        model_gen.train()
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            print('| epoch {:3d} | {:5d}/{:5d} batches | gen lr {:05.8f} | '
                ...
                epoch, batch, len(train_data) // args.bptt, optimizer_gen.param_groups[0]['lr'], cur_loss_reconst))
