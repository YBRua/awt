...
def parse_args_baseline_training():
    parser = ArgumentParser('Baseline Training')
    # dataset arguments
...
def setup_logger_cwm():
    logger = logging.getLogger('Baseline AWT Training')
    ...
    filename = f'awt-baseline-{timestamp}.log'
        ...
        for bid, batch in enumerate(progress):
            ...
            if args.use_semantic_loss:
                if args.codebert:
                    ...
                    orig_sem_emb = sent_encoder(token_ids.permute(1, 0),
                    ...
                    fake_sem_emb = sent_encoder(inputs_embeds=fake_input_emb,
                                                attention_mask=cb_attn_mask).pooler_output
                else:
...
def main(args):
    ...
    # loading data
    processor = CodeWatermarkProcessor(args.lang)

    instances = processor.load_nested_dir(args.data, os.cpu_count() // 2)
    ...
    train_dataset = processor.build_dataset(train_instances, vocab)
    test_dataset = processor.build_dataset(test_instances, vocab)
...
if __name__ == '__main__':
    args = parse_args_baseline_training()
    main(args)
