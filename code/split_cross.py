...
class SplitCrossEntropyLoss(nn.Module):
    ...
    def logprob(self,
        ...
        if softmaxed_head_res is None:
            ...
            if self.nsplits > 1:
                head_weight = self.tail_vectors if head_weight is None else torch.cat(
        ...
        for idx in splits:
            ...
            # If the target is in one of the splits, the probability is the p(tombstone) * p(word within tombstone)
                ...
                tail_res = torch.nn.functional.linear(hiddens,
                                                      tail_weight,
                                                      bias=tail_bias)
    ...
    def forward(self, weight, bias, hiddens, targets, verbose=False):
        ...
        for idx in range(self.nsplits):
            ...
            # If the target is in one of the splits, the probability is the p(tombstone) * p(word within tombstone)
                ...
                tail_entropy = torch.gather(torch.nn.functional.log_softmax(tail_res,
                                                                            dim=-1),
                                            dim=1,
