...
def evaluate(model_gen: TranslatorGeneratorModel,
    ...
    for bid, batch in enumerate(tqdm(dataloader)):
        ...
        with torch.no_grad():
            ...
            tot_forward_sent_enc_time += forward_sent_enc_time
            tot_sampling_time += sampling_time
            tot_gan_time += gan_time
            tot_beam_search_time += beam_time
    ...
    return {
        ...
        'forward_sent_enc_time': tot_forward_sent_enc_time / batch_count,
...
def main(args):
    ...
    if args.vocab_source == 'wikitext':
        ...
        logger.info(f'Loaded train instances: {len(_train_dataset)}')
