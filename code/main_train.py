...
parser.add_argument('--disc_lr', type=float, default=0.0001, help='initial learning rate')
...
parser.add_argument('--scheduler', type=int, default=1, help='whether to schedule the lr')
...
parser.add_argument('--emsize_lm', type=int, default=400, help='size of word embeddings')
...
parser.add_argument('--dropouti_lm',
                    ...
                    default=0.65,
                    help='dropout for input embedding layers (0 = no dropout)')
parser.add_argument('--dropoute_lm',
                    type=float,
                    default=0.1,
                    help='dropout to remove words from embedding layer (0 = no dropout)')
parser.add_argument(
...
parser.add_argument(
    ...
    default='',
    help=
    'path of the autoencoder path to use as init to the generator, in case the model is pretrained as autoencoder only'
)
...
def learing_rate_scheduler():
    ...
    lr = np.power(d_model, -0.8) * min(np.power(step_num, -0.5),
...
def learing_rate_disc_scheduler():
    d_model = args.emsize
    warm_up = args.warm_up
    lr = np.power(d_model, -1.1) * min(np.power(discr_step_num, -0.5),
                                       discr_step_num * np.power(warm_up, -1.5))
    return lr
...
if args.resume:
    ...
    model_gen = model_mt_autoenc_cce.TranslatorGeneratorModel(
        ntokens, args.emsize, args.msg_len, args.msg_in_mlp_layers, args.msg_in_mlp_nodes,
        args.encoding_layers, args.dropout_transformer, args.dropouti, args.dropoute,
        args.tied, args.shared_encoder, args.attn_heads, autoenc_model)
    model_disc = model_mt_autoenc_cce.TranslatorDiscriminatorModel(
        args.emsize, args.adv_encoding_layers, args.dropout_transformer,
...
if args.use_lm_loss:
    with open(args.lm_ckpt, 'rb') as f:
        ...
        langModel = lang_model.RNNModel(args.model, ntokens, args.emsize_lm, args.nhid,
...
###
params = list(model_gen.parameters()) + list(criterion.parameters()) + list(
    criterion_reconst.parameters()) + list(model_disc.parameters())
...
def evaluate(data_source, batch_size=10):
    ...
    for i in range(0, data_source.size(0) - args.bptt, args.bptt):
        ...
        if args.use_semantic_loss:
            ...
            fake_sem_emb = modelSentEncoder.forward_encode_nopad(fake_one_hot,
                                                                 one_hot=True)
...
def train():
    ...
    while i < train_data.size(0) - 1 - 1:
        ...
        errG_msg = criterion(msg_out, msgs.float())
        ...
        if args.use_semantic_loss:
            ...
            fake_sent_emb = modelSentEncoder.forward_encode_nopad(fake_one_hot,
                                                                  one_hot=True)
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            print(
                ...
                'gen loss {:5.2f} | disc loss {:5.2f} | msg loss {:5.2f} | sem loss {:5.2f} | reconst loss {:5.4f} | lm loss {:5.4f}'
                .format(epoch, batch,
                        len(train_data) // args.bptt, optimizer_gen.param_groups[0]['lr'],
                        optimizer_disc.param_groups[0]['lr'],
                        elapsed * 1000 / args.log_interval, cur_loss_gen, cur_loss_disc,
                        cur_loss_msg, cur_loss_sem, cur_loss_reconst, cur_loss_lm))
            total_loss_gen = 0
            total_loss_msg = 0
            total_loss_disc = 0
            total_loss_sem = 0
...
try:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if 't0' in optimizer_gen.param_groups[0]:
            ...
            val_loss_reconst2, val_loss_gen2, val_loss_msg2, val_loss_sem2, val_loss_lm2, val_loss_disc2 = evaluate(
                val_data, eval_batch_size)
            ...
            print(
                ...
                'val disc loss {:5.2f} | val msg loss {:5.2f} | val sem loss {:5.2f} | val reconst loss {:5.2f} | val lm loss {:5.2f}'
                .format(epoch, (time.time() - epoch_start_time), val_loss_gen2,
                        val_loss_disc2, val_loss_msg2, val_loss_sem2, val_loss_reconst2,
                        val_loss_lm2))
            print('-' * 89)
            log_file_loss_val.write(
                str(val_loss_gen2) + ', ' + str(val_loss_disc2) + ', ' +
                str(val_loss_msg2) + ', ' + str(val_loss_sem2) + ', ' +
                str(val_loss_reconst2) + ', ' + str(val_loss_lm2) + '\n')
            ...
            val_loss_reconst, val_loss_gen, val_loss_msg, val_loss_sem, val_loss_lm, val_loss_disc = evaluate(
                val_data, eval_batch_size)
            ...
            print(
                '| end of epoch {:3d} | time: {:5.2f}s | val gen loss {:5.2f} | '
                'val disc loss {:5.2f} | val msg loss {:5.2f} | val sem loss {:5.2f} | val reconst loss {:5.2f} | val lm loss {:5.2f}'
                .format(epoch, (time.time() - epoch_start_time), val_loss_gen,
                        val_loss_disc, val_loss_msg, val_loss_sem, val_loss_reconst,
                        val_loss_lm))
            print('-' * 89)
            log_file_loss_val.write(
                str(val_loss_gen) + ', ' + str(val_loss_disc) + ', ' + str(val_loss_msg) +
                ', ' + str(val_loss_sem) + ', ' + str(val_loss_reconst) + ', ' +
                str(val_loss_lm) + '\n')
            log_file_loss_val.flush()
            ...
            if args.optimizer == 'sgd' and 't0' not in optimizer_gen.param_groups[0] and (
                    len(best_val_loss) > args.nonmono and
                    val_loss > min(best_val_loss[:-args.nonmono])):
                print('Switching to ASGD')
                optimizer_gen = torch.optim.ASGD(model_gen.parameters(),
                                                 lr=args.lr,
                                                 t0=0,
                                                 lambd=0.,
                                                 weight_decay=args.wdecay)
                optimizer_disc = torch.optim.ASGD(model_disc.parameters(),
                                                  lr=args.lr,
                                                  t0=0,
                                                  lambd=0.,
                                                  weight_decay=args.wdecay)
...
# Run on test data.
test_loss_reconst, test_loss_gen, test_loss_msg, test_loss_sem, test_loss_lm, test_loss_disc = evaluate(
    test_data, test_batch_size)

print('-' * 89)
print(
    '| End of training | test gen loss {:5.2f} | test disc loss {:5.2f} | test msg loss {:5.2f} | test sem loss {:5.2f} | test reconst loss {:5.2f} | test lm loss {:5.2f}'
    .format(test_loss_gen, test_loss_disc, test_loss_msg, test_loss_sem,
            test_loss_reconst, test_loss_lm))
print('-' * 89)
